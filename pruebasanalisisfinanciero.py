...
try:
    # Intenta obtener la contraseña del secreto 'general.password'
    real_password = st.secrets["general"]["password"]
except Exception:
    # Muestra un error y detiene la aplicación si el secreto no se encuentra.
...
if password != real_password:
    # Muestra una advertencia y detiene la aplicación si la contraseña es incorrecta.
...
def generate_styled_vertical_analysis_er_table(input_df: pd.DataFrame, total_ingresos_for_cc: float, cuenta_col_original: str, nombre_col_display: str, valor_col_report: str):
    """Genera una tabla de análisis vertical para el Estado de Resultados con estilos."""
    va_df = pd.DataFrame()
...
# Esto asegura que los DataFrames y otros estados se mantengan a través de los reruns.
...
if uploaded_file is not None:
    # --- Cargar y procesar los datos en los dataframes MAESTROS ---
    # Solo procesa el archivo si es nuevo o si los dataframes maestros están vacíos.
    if st.session_state.uploaded_file_name != uploaded_file.name or st.session_state.df_er_master.empty:
        ...
        try:
            ...
            # --- Procesamiento ER (Carga en df_er_master) ---
            er_conf = COL_CONFIG['ESTADO_DE_RESULTADOS']
            bg_conf = COL_CONFIG['BALANCE_GENERAL'] # Se usa en esta sección, aunque es de BG
            ...
            # --- Procesamiento BG (Carga en df_bg_master) ---
...
if report_type == "Estado de Resultados" and df_er_exists:
    # --- Usar la copia de trabajo para todas las operaciones de ER ---
    ...
    if not df_bg_actual.empty and bg_niv_col_slider_disp in df_bg_actual.columns:
        ...
        indicador_liquidez = razon_corriente # Redundante, pero se mantiene por si se quiere un nombre diferente
        ...
        for report_label, active_df_key_search, active_config_key_search in [
            ...
            if not st.session_state.get(active_df_key_search, pd.DataFrame()).empty:
                ...
                if not sub_accounts_df_search.empty:
                    ...
                    st.info(f"No se encontraron subcuentas para '{search_account_input}' en el {report_label}.")

# --- Sección para descargar Excel (al final, para que no interfiera con el flujo principal) ---
if not st.session_state.get('final_er_display', pd.DataFrame()).empty or not st.session_state.get('final_bg_display', pd.DataFrame()).empty:
    excel_buffer = to_excel_buffer(st.session_state.final_er_display, st.session_state.final_bg_display)
